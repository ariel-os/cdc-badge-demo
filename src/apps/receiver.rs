...
/// Receives messages from the GATT server, pressing "0" clears the message history.
...
impl<B: Backend> App<B> {
    ...
    /// Receives the messages in the channel and puts them in an [`heapless::HistoryBuf`].
    async fn handle_messages(
        &self,
        receiver: &Receiver<'_, CriticalSectionRawMutex, GattMessage, 10>,
    ) {
        loop {
    ...
    pub async fn run<'b, M: RawMutex, const CAP: usize, const SUBS: usize, const PUBS: usize>(
        ...
        B::Error: 'static,
    {
        let ble_gatt_receiver = ble::gatt_message_receiver();
