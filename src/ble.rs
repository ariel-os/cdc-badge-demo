...
pub fn scanned_device_receiver<'a>() -> Receiver<'a, CriticalSectionRawMutex, Contact, 32> {
    SCANNED_DEVICES_CHANNEL.receiver()
}

pub fn gatt_message_receiver<'a>() -> Receiver<'a, CriticalSectionRawMutex, GattMessage, 10> {
    TX_CHANNEL.receiver()
}
                        ...
                        match event.payload().incoming() {
                            ...
                            trouble_host::att::AttClient::Request(AttReq::ExecuteWrite {
                                ...
                                match server.write_service.write_data.get(server) {
                                    Ok(message) => {
                                        if let Ok(message) = heapless::String::from_utf8(message) {
    ...
    fn on_adv_reports(&self, mut it: LeAdvReportsIter<'_>) {
        while let Some(Ok(report)) = it.next() {
            ...
            let name = {
                ...
                // Try to find a name in the advertisement data, use `CompleteLocalName` in priority.
                for adv in adv_data {
                    match adv {
            ...
            if let Err(e) = SCANNED_DEVICES_CHANNEL.try_send(c) {
